// apps/api/src/services/parsing/parsingService.ts
...
export class ParsingService {
   ...
   */
  constructor(
    private parsingRepo: ParsingRepository,
...
Please follow this structure for the entire resume, maintaining the integrity of the original text. Provide only the XML output without additional comments and complete the ENTIRE document. The text to parse is: ${text}`;
  ...
  /**
   * Streaming parse with logging
   */
  public async parseWithLoggingAndStreaming(request: ParseRequest): Promise<number> {
    try {
      // Create a new parsing record
      const record = await this.parsingRepo.insert({
      ...
      if (!record?.id) {
        throw new BadRequestError("Failed to create a parsing record");
      }
      ...
      throw new InternalServerError(`Error initiating parsing: ${error.message}`, true);
    }
  }
  ...
  private async processStream(
    ...
    while (retries < this.MAX_RETRIES) {
      try {
        // Request streaming from Anthropic
        ...
        for await (const chunk of stream) {
          if (chunk.type === 'content_block_delta' && 'text' in chunk.delta) {
            ...
            if (updatedProgress !== null) {
              await this.parsingRepo.updateProgress(parsingId, updatedProgress);
  ...
  private async completeProcessing(
    ...
    if (!parsedResume) {
      ...
      await this.parsingRepo.markAsError(parsingId);
      return;
    }

    // Finalize the parsing record
    await this.parsingRepo.finalize(parsingId, annotatedXML);

    // Retrieve the parsing record to obtain the associated documentId
    const parsingRecord = await this.parsingRepo.getById(parsingId);
    ...
    await this.documentRepo.update(parsingRecord.documentId, {
      data: parsedResume,
      isParsed: true,
      inKnowledgeBank: addToKnowledgeBank,
    });

    // If vectorization is required, retrieve the updated document and vectorize positions
    if (addToKnowledgeBank) {
      try {
        const updatedDoc = await this.documentRepo.getById(parsingRecord.documentId);
        if (!updatedDoc) {
   ...
   */
  public async parseSyncOrNoLog(request: ParseRequest): Promise<string> {
    let combinedOutput = "";
    let finishReason: string | null = null;
    let retries = 0;
    ...
    while (retries < this.MAX_RETRIES) {
      try {
        ...
        combinedOutput = response.content.filter(
  ...
  /**
   * Check if XML is complete and well-formed
   */
   ...
   */
  public async getParsingStatusByDocumentId(documentId: number): Promise<ParsingRecord | null> {
    return this.parsingRepo.getLatestByDocumentId(documentId);
  }

  /**
   * Update similar positions
   */
  private async updateSimilarPositions(
    ...
    for (const match of similarMatches) {
      ...
      // Retrieve the matched position
      const matchedPosition = await this.positionRepo.getByUuid(similarUuid);
      if (matchedPosition) {
        ...
        // Update the matched position
        await this.positionRepo.updateFields(matchedPosition.positionUuid, {
          similarPositionUuids: Array.from(updatedSimilar),
        });
      }
    }

    // Combine existing similarPositionUuids with newly found ones
    const updatedSimilar = new Set(currentPosition.similarPositionUuids);
    similarUuids.forEach(uuid => updatedSimilar.add(uuid));

    // Update current position
    await this.positionRepo.updateFields(currentPosition.positionUuid, {
      similarPositionUuids: Array.from(updatedSimilar),
    });
  }

  /**
   * Extract position UUID from a composite ID
   */
  ...
  /**
   * Floor a percentage to the nearest ten
   */
  ...
  /**
   * Determine whether progress should be updated
   */
