  ...
  return (
    <div className="mt-4 border-t pt-4">
      ...
      <div className="mb-2 flex space-x-2">
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange("date", { ...position.date, startDate: e.target.value })}
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => onChange("date", { ...position.date, endDate: e.target.value })}
            className="border p-2 rounded"
          />
        </div>
        <div className="flex items-center mt-6">
          <input
            ...
            onChange={(e) => onChange("date", { ...position.date, present: e.target.checked })}
            className="mr-2"
          />
          <label>Present</label>
        </div>
            ...
            {position.details.activities.map((activity, idx) => (
              <li key={idx} className="bg-gray-100 p-2 rounded-lg">
                <input
                  ...
                  value={activity}
                  onChange={(e) => {
                    const newActivities = [...(position.details?.activities || [])];
                    newActivities[idx] = e.target.value;
                    onChange("details", { ...position.details, activities: newActivities });
                  }}
                  className="border p-2 rounded w-full"
                />
              </li>
      ...
      <div className="mb-2">
        ...
        {position.details?.accomplishments && position.details.accomplishments.length > 0 ? (
          <ul className="list-disc list-inside space-y-2">
            {position.details.accomplishments.map((accomplishment, idx) => (
              <li key={idx} className="bg-gray-100 p-2 rounded-lg">
                <input
                  ...
                  onChange={(e) => {
                    const newAccomplishments = [...(position.details?.accomplishments || [])];
                    newAccomplishments[idx] = e.target.value;
                    onChange("details", { ...position.details, accomplishments: newAccomplishments });
                  }}
                  className="border p-2 rounded w-full"
                />
              </li>
